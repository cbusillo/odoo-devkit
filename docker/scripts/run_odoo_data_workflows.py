...
def _module_is_installed(env, module_name):
...
def _find_website_page(env, website, *, xmlid, url):
    page = None
    if xmlid:
...
def _verify_route(env, website, route_payload, *, fallback_module):
    ...
    match = getattr(ir_http, '_match', None)
    if callable(match):
        try:
            match(route_url)
            return None
...
def _apply_website_bootstrap(env, parsed_payload):
    ...
    if homepage_url and not homepage_page:
        _verify_route(env, website, {'url': homepage_url, 'module': fallback_module, 'published': True}, fallback_module=fallback_module)
...
with registry.cursor() as cr:
    ...
    _apply_website_bootstrap(env, instance_override_payload)
    cr.commit()
